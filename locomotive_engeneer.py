...
def add_missing_stops(route: dict, **kwargs):
    ...
    """
    stops = [stop for stop in kwargs.values()]
    route["stops"] = stops
    return route
...
def fix_wagon_depot(wagons_rows):
    ...
    """
    new_list = [[] for _ in range(len(wagons_rows))]
    for x in range(len(wagons_rows)):
        for y in range(len(wagons_rows)):
            new_list[y].append(wagons_rows[x][y])
    return new_list
